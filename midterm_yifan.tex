...
Description of experts' background knowledge.
\end{column}
\end{columns}
...
\begin{center}
\begin{tikzpicture}[scale=0.7,transform shape,every text node part/.style={align=center},level 1/.style={level distance=1.5cm, sibling distance=9cm}, level 2/.style={level distance=2cm, sibling distance=6cm},
...
\begin{frame}{Minimal hitting set}
...
Six branches are open in the tableau at the end. In each one, the set of concepts that cannot be decomposed are:
 ...
 \end{align*}
 \begin{center}
\begin{tikzpicture}[scale=0.4, transform shape, every text node part/.style={align=center},level 1/.style={level distance=2cm,sibling distance=8cm},level 2/.style={level distance=1cm, sibling distance=2.5cm}]
...
\end{itemize}
 \end{itemize}
\end{block}
% \begin{defn}[Subsumption criterion]
% For an abduction problem $\mathcal{P}=\langle \mathcal{T},\mathcal{H}, \mathcal{O}\rangle$, and $\{\langle P_{1},\dots,P_{n}\rangle\}$ potential hypotheses,
% \begin{itemize}
% \item  $P_{i}$ is a $\sqsubseteq -minimal$ explanation if there does not exist an explanation $P_{j}$ for $\mathcal{P}$ such that $P_{i}\sqsubseteq P_{j}$.
% \end{itemize}
